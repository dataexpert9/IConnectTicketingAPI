...
namespace BLL.Implementation
{
    ...
    //    {
    //        return _dbContext.RequestItem.Include(x => x.RequestItemML).Include(x => x.RequestItemImages).Include(x => x.Driver).Where(x => x.Status == 1 && x.IsDeleted == false).Skip(Items * Page).Take(Items).OrderByDescending(x => x.Id).ToList();
    //    }

    //    public List<RequestItem> GetRiderRequests(int Driver_Id, int Items, int Page)
    //    {
    //        return _dbContext.RequestItem.Include(x => x.RequestItemML).Include(x => x.RequestItemImages).Include(x => x.Driver).Where(x => (x.Status == 1 || x.Status == 1 || x.Status ==1) && x.IsDeleted == false && x.Driver_Id == Driver_Id).Skip(Items * Page).Take(Items).OrderByDescending(x => x.Id).ToList();
    //    }
    ...
    //        var item = _dbContext.RequestItem.FirstOrDefault(x => x.Id == Request_Id && x.Status == 1 && x.IsDeleted == false);
    //        if (item != null)
    //        {
    //            item.Status = 1;
    ...
    //    {
    //        var Request = _dbContext.RequestItem.Include(x => x.RequestItemML).Include(x => x.Driver).Include(x => x.RequestItemImages).FirstOrDefault(x => x.Id == Request_Id && x.Status == 1);
    ...
    //    {
    //        var Request = _dbContext.RequestItem.FirstOrDefault(x => x.Id == Request_Id && x.Driver_Id == Driver_Id && x.Status == 1);
    //        if (Request != null)
    //        {
    //            Request.Status = 1;
    //            _dbContext.SaveChanges();
